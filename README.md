...
* [基于 `Page Object` 设计模式的企业微信 `web`端自动化测试实战](./web自动化测试/selenium_wework_main)
  ...
  > 环境：platform win32 -- Python 3.8.6, pytest-7.0.1, pluggy-1.0.0 chrome 版本为 98.0.4758.102（正式版本）（64 位）
  >
  > chrome 驱动镜像下载地址为：https://registry.npmmirror.com/binary.html?path=chromedriver/ 根据浏览器版本下载对应版本驱动
  >
  > chrome 驱动中没有 win64，仅有 win32，但在64位 windows 系统上可以兼容
...
2. 显示等待和隐式等待
   ...
   * 显示等待的代码定义了等待条件，只有该条件触发，才执行后续代码，可以把 sleep() 看作一种人为固定等待时间的显示等待

   * 隐式等待是在尝试发现元素的时候，如果没能立刻发现，则在设置的最大等待时间间隔内不断轮询查找，发现则立即进行下一步操作，达到最大等待时间未发现则抛出异常。一旦设置了隐式等待时间，它的作用范围就是 WebDriver 对象实例的整个生命周期。即，对于每个 WebDriver 对象实例，仅需要设置一次隐式等待时间，就会对该实例进行的查找元素操作均有效
